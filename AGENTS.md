...
- Write GitHub PR titles and bodies in English.
...
Project: wiki-llm-starter
Methodology: ddd
Agent mode: single

## LLM Wiki Skills

These skills are available for all Codex sessions in this repo.
Call them with `$skill-name` syntax or by typing the skill name in your prompt.

| 스킬 | Codex 호출 | 역할 |
|------|-----------|------|
| wiki-ingest | `$wiki-ingest` | raw/ 자료 → wiki 페이지 + ontology 업데이트 + gbrain 동기화 |
| wiki-query | `$wiki-query` | wiki 기반으로 질문에 답변 |
| wiki-lint | `$wiki-lint` | 위키 구조·링크·출처 건강검진 |
| ontology-check | `$ontology-check` | T-Box/A-Box 일관성 및 추론 검증 |
| vibe-wiki-session | `$vibe-wiki-session` | 세션 시작 시 wiki 맥락 불러오기 + 세션 종료 시 저장 |
| wiki-release | `$wiki-release` | 배포용 ZIP 패키징 및 검증 |

### Codex에서 gbrain 사용하기

Codex 환경에서는 gbrain CLI를 터미널에서 직접 실행합니다.
MCP 서버는 Claude Code 전용입니다. Codex에서는 아래처럼 씁니다:

```bash
# 위키 페이지 검색 (터미널)
gbrain search "질문 키워드"

# Ingest 후 동기화
gbrain import wiki/
```

스킬 내부에서 이 명령이 자동 실행되므로 별도로 호출할 필요는 없습니다.
